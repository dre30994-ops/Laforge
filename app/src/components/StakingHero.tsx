"use client";

import Link from "next/link";
import { CreatePoolButton } from "@/components/CreatePoolButton";

/**
 * Dashboard hero band.
 *
 * Left column: a short value-proposition about staking, a primary
 * "Create Stake" call-to-action, and a secondary "How it works" link that
 * routes to the docs page.
 *
 * Right column: the Forge mark (icon2_nobg.png) centered inside a slowly
 * rotating ring of placeholder icon chips that encircle it. The ring spins
 * while each chip counter-rotates so the glyphs stay upright.
 */
export function StakingHero() {
  return (
    <section className="glass glass-gold p-6 md:p-8 animate-rise overflow-hidden">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        {/* ── Left: message + CTAs ── */}
        <div className="min-w-0">
          <span className="label-term text-pos">Stake · earn · compound</span>
          <h2
            className="mt-2 text-2xl md:text-3xl font-semibold tracking-tight leading-tight
                       bg-clip-text text-transparent"
            style={{
              backgroundImage:
                "linear-gradient(90deg, #b8860b 0%, #ffcf4d 35%, #fff6d6 70%, #ffffff 100%)",
            }}
          >
            Put your tokens to work.
          </h2>
          <p className="mt-3 text-sm md:text-base text-mid leading-relaxed max-w-md">
            Earn continuous, auto-accruing yield with a tenure bonus that grows the
            longer you stake — up to 2.0&times;. No lockups you can&apos;t exit, transparent
            emissions, and rewards that compound in real time.
          </p>

          <div className="mt-6 flex flex-wrap items-center gap-3">
            <CreatePoolButton label="Create Stake" />
            <Link
              href="/docs"
              className="inline-flex items-center justify-center gap-2 h-10 px-5 rounded-xl
                         text-sm font-semibold text-hi border border-black/15
                         hover:border-black/25 hover:bg-black/[0.04] transition-colors"
              style={{ fontFamily: "var(--font-mono, monospace)" }}
            >
              How it works
              <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor"
                strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                <path d="M5 12h14" />
                <path d="M13 6l6 6-6 6" />
              </svg>
            </Link>
          </div>
        </div>

        {/* ── Right: central mark encircled by placeholder icons ── */}
        <div className="grid place-items-center">
          <OrbitMark />
        </div>
      </div>
    </section>
  );
}

/**
 * Orbit chips. Two slots carry real chain artwork (Ethereum + Robinhood); the
 * other four are artistic "?" glyphs — mysterious placeholders for chains yet
 * to be announced. ETH and Robinhood sit on opposite sides of the ring for
 * visual balance.
 */
type OrbitChip =
  | { kind: "image"; src: string; alt: string }
  | { kind: "question" };

const ORBIT_CHIPS: OrbitChip[] = [
  { kind: "image", src: "/Ethereum--Streamline-Simple-Icons.svg", alt: "Ethereum" },
  { kind: "question" },
  { kind: "question" },
  { kind: "image", src: "/roadmap/Robinhood--Streamline-Simple-Icons.svg", alt: "Robinhood" },
  { kind: "question" },
  { kind: "question" },
];

function OrbitMark() {
  const radius = 118; // px from center to each chip
  const count = ORBIT_CHIPS.length;

  return (
    <div className="relative w-[300px] h-[300px] max-w-full grid place-items-center">
      {/* Soft glow behind the mark */}
      <div
        aria-hidden
        className="absolute w-40 h-40 rounded-full"
        style={{
          background:
            "radial-gradient(circle, rgba(255,207,77,0.22) 0%, rgba(255,207,77,0) 70%)",
        }}
      />

      {/* Dashed orbit ring guide */}
      <div
        aria-hidden
        className="absolute rounded-full border border-dashed border-black/10"
        style={{ width: radius * 2, height: radius * 2 }}
      />

      {/* Rotating ring of chips (counter-rotated to stay upright) */}
      <div
        aria-hidden
        className="absolute inset-0"
        style={{ animation: "hero-orbit 40s linear infinite" }}
      >
        {ORBIT_CHIPS.map((chip, i) => {
          const angle = (i / count) * 2 * Math.PI - Math.PI / 2;
          const x = Math.cos(angle) * radius;
          const y = Math.sin(angle) * radius;
          const isQuestion = chip.kind === "question";
          return (
            <span
              key={i}
              className="absolute left-1/2 top-1/2 grid place-items-center w-11 h-11 rounded-xl select-none"
              style={{
                transform: `translate(-50%, -50%) translate(${x}px, ${y}px)`,
                background: isQuestion
                  ? "radial-gradient(circle at 30% 25%, rgba(255,207,77,0.12), rgba(20,18,10,0.05))"
                  : "rgba(20,18,10,0.04)",
                border: isQuestion
                  ? "1px solid rgba(255,207,77,0.35)"
                  : "1px solid var(--hairline)",
                backdropFilter: "blur(6px)",
                boxShadow: isQuestion
                  ? "0 0 16px rgba(255,207,77,0.18), inset 0 0 10px rgba(255,207,77,0.08)"
                  : "none",
              }}
            >
              <span
                className="grid place-items-center"
                style={{ animation: "hero-orbit-counter 40s linear infinite" }}
              >
                {chip.kind === "image" ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={chip.src}
                    alt={chip.alt}
                    className="w-6 h-6 object-contain"
                    style={{ filter: "drop-shadow(0 0 6px rgba(255,207,77,0.35))" }}
                  />
                ) : (
                  <span
                    className="font-semibold text-xl leading-none bg-clip-text text-transparent animate-gradient-move"
                    style={{
                      backgroundImage:
                        "linear-gradient(90deg, #b8860b 0%, #ffcf4d 35%, #fff6d6 70%, #ffffff 100%)",
                      backgroundSize: "200% auto",
                      fontFamily: "var(--font-serif, Georgia, serif)",
                    }}
                  >
                    ?
                  </span>
                )}
              </span>
            </span>
          );
        })}
      </div>

      {/* Central mark */}
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img
        src="/icon2_nobg.png"
        alt="Forge"
        className="relative z-[1] w-28 h-28 md:w-32 md:h-32 object-contain drop-shadow-[0_0_28px_rgba(255,207,77,0.35)]"
      />

      {/* Orbit keyframes (scoped globally, cheap to include once). */}
      <style>{`
        @keyframes hero-orbit {
          from { transform: rotate(0deg); }
          to { transform: rotate(360deg); }
        }
        @keyframes hero-orbit-counter {
          from { transform: rotate(0deg); }
          to { transform: rotate(-360deg); }
        }
      `}</style>
    </div>
  );
}
